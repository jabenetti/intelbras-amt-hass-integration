...
class AMTServer:
    ...
    async def _handle_connection_info(
        ...
        if len(content) < 6:
            _LOGGER.error("Connection info data too short: %d bytes", len(content))
            return

        # Byte 0: channel
        # Bytes 1-2: account (nibbles, where 0xA represents digit 0)
        # Bytes 3-5: partial MAC (last 3 bytes)
        channel = content[0]
        account_bytes = content[1:3]
        mac_bytes = content[3:6]

        def _decode_account(b: bytes) -> str:
            """Decode account number where 0xA nibble means digit 0."""
            result = []
            for byte in b:
                high = byte >> 4
                low = byte & 0x0F
                result.append("0" if high == 0x0A else f"{high:X}")
                result.append("0" if low == 0x0A else f"{low:X}")
            return "".join(result)

        connection.account = _decode_account(account_bytes)
        connection.mac_suffix = mac_bytes.hex().upper()

        _LOGGER.info(
            "Panel connected: IP=%s, port=%i, account=%s, mac=%s",
    ...
    def _parse_battery_level(self, battery_byte: int):
        """Parse battery level to percentage"""
        levels = 0
        if battery_byte & 0x02:
            levels += 1
        if battery_byte & 0x04:
            levels += 1
        if battery_byte & 0x08:
            levels += 1

        level_to_pct = [0, 33, 66, 100]
        return level_to_pct[levels]
    ...
    def _parse_response(self, data: bytes) -> dict[str, Any]:
        ...
        if self._last_status is None:
            _LOGGER.info("Panel identified: %s", model_name)
        ...
        # Battery level
        battery_byte = content[40] if long_response else content[30]
        battery_level = self._parse_battery_level(battery_byte)
